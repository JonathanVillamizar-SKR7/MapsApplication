...
data class PermissionManagerState(
    val status: PermissionStatus, val requestPermissions: () -> Unit
)
...
fun rememberPermissionManager(permission: AppPermission): PermissionManagerState {
    ...
    LaunchedEffect(Unit) {
        // Al iniciar, comprobamos si todos los permisos ya estaban concedidos
    ...
    return PermissionManagerState(
        status = status, requestPermissions = ::requestPermissions
    )
}